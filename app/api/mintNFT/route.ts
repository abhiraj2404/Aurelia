...
import { chain, client } from "@/config/client";
...
export async function POST(req: NextRequest, res: NextResponse) {
  if (req.method !== "POST") {
    return NextResponse.json({ error: "Failed to mint NFT" }, { status: 405 });
  }

  try {
    // Connect to the provider and initialize Thirdweb SDK
    ...
    if (!voucher || !account) {
      return NextResponse.json(
        { error: "voucher and account are required" },
        { status: 400 }
      );
    }
    const types = {
    ...
    // Call the `safeMint` function with the voucher and user address
    ...
    return NextResponse.json({ error: "Failed to mint NFT" }, { status: 200 });
  }
}
